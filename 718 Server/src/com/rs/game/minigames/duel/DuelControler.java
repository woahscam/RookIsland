...
public class DuelControler extends Controler {
	...
	public static void challenge(Player player) {
		...
		if (player.getIronman().isIronman()) {
			player.sm("You cannot duel as a " + (player.getIronman().Ironman_Mode == 2 ? "HC ironman." : "Ironman."));
			return;
		}
		if (target.getIronman().isIronman()) {
			player.sm("You cannot duel a " + (target.getIronman().Ironman_Mode == 2 ? "HC ironman." : "Ironman."));
			return;
		}
