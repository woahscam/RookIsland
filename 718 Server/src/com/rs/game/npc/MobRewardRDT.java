...
public final class MobRewardRDT {
	 ...
	 * 
	 * @param row Is the player wearing a ring of wealth.
	 * @return the generated item or null if player is unlucky.
	 */
	public Item generateReward(Player player) {
		boolean row = ItemDefinitions.getItemDefinitions(player.getEquipment().getRingId()).getName().toLowerCase().contains("ring of wealth");
		//A ring of wealth affects how the rare drop table works. 
		//It removes most of the "empty" slots on the rare drop table. 
		//Therefore, the odds of receiving items is increased. 
		//A ring of wealth does not affect how often the rare drop table is accessed. 
		//So, when wearing a ring of wealth, the rare drop table will not be triggered more often, 
		//but when it is triggered, the player is more likely to receive an item.
		final int lucky = row ? 2 : 1;
		//128
		if (Utils.random(128) <= lucky) {
			...
			if (node.size() <= 0) {
				System.out.println("error node size <= 1");
				return null;
			}
			final int shake = Utils.random(node.size());
			player.getPackets().sendGameMessage("<col=ff0000>You got a rare drop table item.");
			return node.get(shake).getReward().generateItem(); //gets an item!
		}
		else {
			return null;
		}
