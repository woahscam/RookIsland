...
public abstract class CombatScript {
	...
	public static int getRandomMaxHit(NPC npc, int maxHit, int attackStyle, Entity target) {
		...
		if (attackStyle == NPCCombatDefinitions.MAGE) {
			...
			A = Math.round(attack);
			System.out.println(npc.getName() + " : " + A);
		}
		double defence = 0;
		double D = 0;
		...
		if (target instanceof Player) {
			...
			if (attackStyle == NPCCombatDefinitions.MAGE) {
				...
				D = Math.round(defence);
				System.out.println(npc.getName() + " DEFENCE: " + D);
			}
		}
		if (attackStyle == NPCCombatDefinitions.MAGE) {
			double prob = M / MD;
			...
			double prob = A / D;
			double random = Utils.getRandomDouble(100);
			System.out.println(npc.getName() + " prob: " + prob);
			System.out.println(npc.getCombatDefinitions().getMaxHit());
			if (A <= D)
				prob = (A - 1) / (D * 2);
			else if (A > D)
				prob = 1 - (D + 1) / (A * 2);
