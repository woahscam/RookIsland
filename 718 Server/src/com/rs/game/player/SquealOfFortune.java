...
public class SquealOfFortune implements Serializable {
	...
	public void processClick(int packetId, int interfaceId, int componentId, int e1, int e2) {
		if (interfaceId == 1139) { // squeal tab
			...
			if (componentId == 5) {
				...
				player.getInterfaceManager().closeTab(player.getInterfaceManager().hasRezizableScreen(), player.getInterfaceManager().hasRezizableScreen() ? 11 : 0);
			} else {
				openSpinInterface();
			}
	...
	public void giveDailySpins() {
		...
		dailySpins += player.getPlayerMode() + 1;
