...
public class SlayerManager implements Serializable {
				...
				if (player.getSkills().getLevel(Skills.SLAYER) >= task.getLevelRequried()
					...
					return new Object[] { task,
							(task == SlayerTask.CORPOREAL_BEAST ? Utils.random(5,
									10)
									: Utils.random(currentMaster.getTasksRange()[0],
	...
	public boolean isValidTask(String name) {
		...
		for (SlayerTask currentTask : tasks) {
			if (name.toLowerCase()
					.equalsIgnoreCase(currentTask.toString().replace("_", " ").replace("$", "'").toLowerCase()))
				return true;
			if (name.toLowerCase().contains(currentTask.toString().replace("_", " ").replace("$", "'").toLowerCase()))
				return true;
		}
		return false;
	}
