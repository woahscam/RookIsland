...
public class Mining extends Action {
	...
	public int processWithDelay(Player player) {
		...
		if (Utils.getRandom(realOre.randomLife) == 0) {
			World.removeObjectTemporary(player, rock, realOre.respawnDelay * 600, false);
			WorldObject emptySpot = new WorldObject(realOre.emptySpot, rock.getType(), rock.getRotation(), rock);
			World.spawnObjectTemporary(player, emptySpot, realOre.respawnDelay * 600);
	...
	private Gems calculateGem(boolean gemRock) {
		while (true) {
			...
			double chanceBoost = !gemRock ? 15.0 : 0;
			Gems gem = Gems.values()[Utils.getRandom(Gems.values().length - 1)];
			System.out.println(ItemDefinitions.getItemDefinitions(gem.getItemId()).getName() + ", gemRockOnly?: "
					+ gem.isGemRockOnly() + ", chance: " + (gem.getChance() + chanceBoost) + "%, roll: " + chance + " "
					+ ((gem.getChance() + chanceBoost) >= chance
							? (!gemRock && gem.isGemRockOnly() ? "success, but gemrockonly gem, reroll" : "success!")
							: "failed, reroll!"));
