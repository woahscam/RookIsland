...
public class HintIconsManager {
	...
	public int addHintIcon(NPC npc, int arrowType, int modelId, boolean saveIcon) {
		int index = saveIcon ? getFreeIndex() : 7;
		if (index != -1) {
			HintIcon icon = new HintIcon(npc.getIndex(), npc instanceof NPC ? 10 : 1, arrowType, modelId, index);
			player.getPackets().sendHintIcon(icon);
			if (saveIcon)
				loadedIcons[index] = icon;
		}
