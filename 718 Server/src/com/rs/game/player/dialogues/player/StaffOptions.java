...
public class StaffOptions extends Dialogue {
	...
	void undoPunishment(Object targetInfo, boolean online, String punishment) {
		if (!online) {
			...
			case "jail":
				player.getPackets().sendGameMessage("You have lifted the jail for %s",
						Utils.formatPlayerNameForDisplay(accName));
				targ.liftJail(false, player);
				// targ.sendLoginMessage("You have been let out of jail by
				// <col=800000>" + player.getDisplayName());
				targ.setNextWorldTile(new WorldTile(3192, 9826, 0));
			...
			switch (punishment.toLowerCase()) {
			case "jail":
				player.getPackets().sendGameMessage("You have lifted the jail for %s", target.getDisplayName());
				target.getPackets().sendGameMessage(
						"You have been let out of jail by <col=800000>" + player.getDisplayName() + "</col>.");
				target.liftJail(false, player);
				break;
			case "mute":
