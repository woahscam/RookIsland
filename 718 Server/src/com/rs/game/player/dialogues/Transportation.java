...
public class Transportation extends Dialogue {
	...
	public void run(int interfaceId, int componentId) {
		...
		if (componentId == OPTION_1) {
			teleported = OldMagicSystem.sendJewerlyTeleportSpell(player, true, EMOTE, GFX, 4, (WorldTile) parameters[1]);
			...
			teleported = OldMagicSystem.sendJewerlyTeleportSpell(player, true, EMOTE, GFX, 4, (WorldTile) parameters[3]);
		} else if (componentId == OPTION_3) {
			teleported = OldMagicSystem.sendJewerlyTeleportSpell(player, true, EMOTE, GFX, 4, (WorldTile) parameters[5]);
		} else if (componentId == OPTION_4) {
			teleported = OldMagicSystem.sendJewerlyTeleportSpell(player, true, EMOTE, GFX, 4, (WorldTile) parameters[7]);
		}
		if (!teleported) {
			end();
			return;
