	...
	public void run(int interfaceId, int componentId) {
		if (stage == 1) {
			if (componentId == OPTION_1) {
				...
				OldMagicSystem.sendNormalTeleportSpell(player, 0, 0, new WorldTile(3507, 9493, 0));// Kalphite
																							// Queen
				player.getInterfaceManager().closeChatBoxInterface();
			} else if (componentId == OPTION_3) {
				OldMagicSystem.sendNormalTeleportSpell(player, 0, 0, new WorldTile(2966, 4383, 2));// corp
																							// tele
				player.getInterfaceManager().closeChatBoxInterface();
			} else if (componentId == OPTION_4) {
				OldMagicSystem.sendNormalTeleportSpell(player, 0, 0, new WorldTile(2562, 5739, 0));// Torm
																							// Demons
				player.getInterfaceManager().closeChatBoxInterface();
