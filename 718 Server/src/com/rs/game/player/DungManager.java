...
public class DungManager implements Serializable {
		...
		for (ResourceDungeon dung : ResourceDungeon.values()) {
			if (object.getId() == dung.outsideId || object.getId() == dung.insideId) {
				...
				if (dung == ResourceDungeon.POLYPORE_DUNGEON)
					OldMagicSystem.sendTeleportSpell(player, 13288, 13285, 2516, 2517, 0, 0, object.getX() == 4695 && object.getY() == 5626 ? dung.outside : dung.inside, 1, false, OldMagicSystem.OBJECT_TELEPORT);
					else
					OldMagicSystem.sendTeleportSpell(player, 13288, 13285, 2516, 2517, 0, 0, object.getId() == dung.insideId ? dung.outside : dung.inside, 1, false, OldMagicSystem.OBJECT_TELEPORT);
	...
	public void invite(String name) {
		...
		if (party == null) {
			...
			for (int i = 0; i < 5; i++) {
				...
				player.getPackets().sendCSVarInteger(1168 + i, teamMate == null ? -1 : teamMate.getSkills().getTotalLevel(teamMate));
			}
	...
	public void enterDungeon(boolean selectSize) {
		player.getPackets().sendGameMessage("enter dung");
		player.stopAll();
		expireInvitation();
		if (party == null) {
		...
		/*if (!party.hasShownCustomSeedDialog()) {
			...
			player.getDialogueManager().startDialogue("CustomSeedD");
			return;
		}*/
