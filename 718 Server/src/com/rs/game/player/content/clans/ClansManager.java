...
public class ClansManager {
	...
	public void refreshAllMapDots() {
		synchronized (this) {
			for (Player member : membersOnline)
				member.setNextClanMemberUpdate("i");
		}
	}
	...
	public void sendMessage(Player player, String message) {
		synchronized (this) {
			...
			int rights = player.getRights();
			for (Player p2 : channelPlayers)
				p2.getPackets().receiveClanChatMessage(membersOnline.contains(p2), displayName, rights, fixed);
		}
	}
	...
	public static void createClan(Player player, String clanName) {
		for (String s : bannedWords) {
			if (clanName.toLowerCase().contains(s) && !player.isAdministrator()) {
				player.getPackets().sendGameMessage("The clan name you tried contains banned words.");
				return;
			}
		}
		if (clanName.length() < 4 && !player.isAdministrator()) {
