...
public class DwarfMultiCannon {
	...
	public static int[] CANNON_PIECES = { 6, 8, 10, 12 };
	public static int[] CANNON_OBJECTS = { 7, 8, 9, 6 };
	...
	public static void setUp(Player player) {
		...
		if (controler != null && !(controler instanceof WildernessControler)
				&& !(controler instanceof CorpBeastControler)) {
		...
		if (!World.isTileFree(pos.getPlane(), pos.getX(), pos.getY(), 3) || World.getStandartObject(player) != null
			...
			public void process(Player player, WorldObject currentObject) {
				...
				if (nearestN != null) {
					...
					World.sendProjectile(currentObject.transform(objectSizeX / 2, objectSizeY / 2, 0), nearestN, 53, 38,
							38, 30, 0, 0, 0);
					nearestN.applyHit(new Hit(player, damage, HitLook.CANNON_DAMAGE,
							Utils.getDistance(
					...
					if (Utils.getRandom(100) < 25) {
						...
						World.sendProjectile(currentObject.transform(objectSizeX / 2, objectSizeY / 2, 0), nearestN, 53,
								38, 38, 30, 0, 0, 0);
						nearestN.applyHit(new Hit(player, damage, HitLook.CANNON_DAMAGE, Utils.getDistance(
								new WorldTile(currentObject.getX(), currentObject.getY(), currentObject.getPlane()),
								nearestN)));
						if (player.getCannonBalls() > 0)
						player.addCannonBalls(-1);
	...
	private static boolean rollCannonHit(Player player, NPC npc) {
		...
		double defenceBonus = npc.getBonuses()[CombatDefinitions.NPC_RANGE_DEFENCE] * 2;
		double defence = 0;
		double D = 0;
		defence = npc.getBonuses()[CombatDefinitions.NPC_DEFENCE_LEVEL];
		...
		if (Settings.DEBUG) {
			System.out
					.println(npc.getName() + ", DefenceLevel: " + npc.getBonuses()[CombatDefinitions.NPC_DEFENCE_LEVEL]
							+ ", RangeDefence: " + npc.getBonuses()[CombatDefinitions.NPC_RANGE_DEFENCE]);
			System.out.println(player.getDisplayName() + ", AttackLevel " + attackLevel + ", bestBonus = "
					+ (melee >= range ? "melee" : "range") + " at " + bestBonus);
			System.out.println("Probability to hit: " + new DecimalFormat("##.##").format(prob * 100) + "%, Roll: "
					+ new DecimalFormat("##.##").format(random) + ", Sucess hit?:"
					+ (prob < random / 100 ? false : true));
		}
		if (prob < random / 100)
			return false;
		return true;
