...
public class DungeonManager {
	...
	public void setResources(Room room, RoomReference reference, int toChunkX, int toChunkY) {
		...
		if (party.getComplexity() >= 3 && Utils.random(3) == 0) { // sets rock
			for (int i = 0; i < DungeonConstants.SET_RESOURCES_MAX_TRY; i++) {
				...
				if (!World.isFloorFree(0, toChunkX * 8 + x, toChunkY * 8 + y))
					continue;
				// TODO World.spawnObject(new
				// WorldObject(DungeonUtils.getMiningResource(Utils.random(DungeoneeringMining.DungeoneeringRocks.values().length),
	...
	public int[] getBonuses(boolean boss, int level) {
		...
		// usualy range def > melee def for mobs but bows in dung suck, and even range
		// gear doesnt help that much
		bonuses[CombatDefinitions.NPC_RANGE_DEFENCE] = (int) ((npcDefenceLevel + (level / (boss ? 2 : 3))) / 1.6); // 60%
																													// nerf

		// less
		// than
		// range
		bonuses[CombatDefinitions.NPC_MAGIC_DEFENCE] = (int) ((npcDefenceLevel + (level / (boss ? 2 : 3))) / 1.5); // 50%
		// less
		// than
		// range
		if (Settings.DEBUG)
	...
	public void loadRewards() {
		...
		for (Player player : party.getTeam()) {
			...
			for (int i = 0; i < 5; i++) {
				...
				player.getPackets().sendCSVarInteger(1198 + i, partyPlayer != null ? 1 : 0); // sets true that this
																								// player exists
				if (partyPlayer == null)
					continue;
			...
			player.getPackets().sendIComponentText(933, 331,
					Utils.getFormatedTimeShort3((Utils.currentTimeMillis() - time) * 600));
			...
			double levelDiffPenalty = party.getLevelDiferencePenalty(player);// party.getMaxLevelDiference() > 70 ?
			...
			// base xp is based on a ton of factors, including opened rooms, resources
			// harvested, ... but this is most imporant one
			...
			int tokens = (int) ((int) totalXp * 1.5); // (int) (totalXp / 10); x10 tokens cuz x10 xp
			player.getPackets().sendCSVarInteger(1237, (int) (floorXP * 100)); // x 10. .x100 cuz x10 server
			player.getPackets().sendCSVarInteger(1238, (int) (prestigeXP * 100));
			player.getPackets().sendCSVarInteger(1239, (int) (averageXP * 100));
			if (Settings.DEBUG)
				System.out.println(averageXP + ", " + multiplier + ", " + totalXp);
			if (hsType != 2) {
				player.getSkills().addXp(Skills.DUNGEONEERING, totalXp); // force rs xp, cuz we do * xp rate in calcs to
																			// make inter show correct xp
			...
			// Highscores.updateDung(this, dungeon, (Utils.currentWorldCycle() - time) *
			// 600L, timems, uid, teamSizeOrig, hsType, seed, player);
		}
		clearGuardians();
	}
