...
public class GrandExchangeManager implements Serializable {
	...
	public void setExtraDetails(int id) {
		...
		if (getType() == 1 || getType() == -1) {
			...
			if (Settings.ECONOMY_MODE >= 1 && GrandExchange.getPrice(id) == 0) {
			...
			} else if (Settings.ECONOMY_MODE == 0 && GrandExchange.getPrice(id) < Settings.LOWPRICE_LIMIT) {
				player.getPackets().sendIComponentText(105, 143,
						ItemExamines.getExamine(item) + "<br> <br>The Grand Exchange will automatically buy this<br>item for 5% less than its guide price.");
			} else {
			if (totalbuyamount > 0) {
				player.getPackets().sendIComponentText(105, 143,
				...
				player.getPackets().sendIComponentText(105, 143,
						ItemExamines.getExamine(item) + "<br> <br>There are currently no buy offers for this item.");
				}
			}
		}
		if (getType() == 0) {
	...
	public void confirmOffer() {
		...
		if (buy) {
			...
			if (Settings.ECONOMY_MODE >= 1 && GrandExchange.getPrice(itemId) == 0) {
