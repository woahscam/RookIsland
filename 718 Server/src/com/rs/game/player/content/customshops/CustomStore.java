	...
	public void sendItemAndInfo(int container) {
		...
		if (TYPE == COINS) {
			if (container >= getItems(TYPE, SHOP).size())
				return;
			int price = (SHOP == PK_POINTS || SHOP == AVALON_POINTS ? getItems(TYPE, SHOP).get(container).getAmount() : EconomyPrices.getPrice(getItems(TYPE, SHOP).get(container).getId()));
	...
	public void sendInfo(int itemId) {
		...
		if (TYPE == SKILLCAPE) {
			...
			for (Item item : getItems(TYPE, SHOP))
				if (itemId == item.getId()) {
					price = SHOP == PK_POINTS || SHOP == AVALON_POINTS ? item.getAmount() : EconomyPrices.getPrice(itemId);
					player.getPackets()
							.sendGameMessage(ItemDefinitions.getItemDefinitions(itemId).getName()
									+ (price == 0 ? " is " + HexColours.getShortMessage(Colours.GREEN, "free!")
											: (" costs "
	...
	public void sendBuy(int itemId, int amount) {
		...
		if (TYPE == SKILLCAPE) {
			...
			for (Item item : getItems(TYPE, SHOP)) {
				if (item.getId() == itemId) {
					canBuy = true;
					defaultPrice = SHOP == PK_POINTS || SHOP == AVALON_POINTS ? item.getAmount() : EconomyPrices.getPrice(itemId);
					points = (SHOP == PK_POINTS ? player.getPKP() : SHOP == AVALON_POINTS ? player.getAvalonPoints() : -1);
					continue;
				}
			}
		} else {
