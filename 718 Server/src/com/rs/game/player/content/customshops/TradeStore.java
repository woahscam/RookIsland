...
public class TradeStore {
	...
	public void removeItem(final int slot, int amount) {
		synchronized (this) {
			...
			int wealth = 0;
			rewardItems.clear();
			for (Item tradeItems : items.getItems()) {
				if (tradeItems == null)
					continue;
			...
			refreshItems(rewardItems.getItems());
	...
	public void addItem(int slot, int amount) {
		synchronized (this) {
			...
			refreshItems(rewardItems.getItems(), false);
			cancelAccepted();
		}
	}
	...
	public void refresh(int... slots) {
		...
		player.getPackets().sendUpdateItems(90, true, rewardItems.getItems(), slots);
	}
	...
	public void closeTrade(CloseTradeStage stage) {
		...
		synchronized (this) {
			...
			if (CloseTradeStage.DONE != stage) {
				...
				for (Item tradedItems : rewardItems.getItems()) {
					if (tradedItems == null)
						continue;
