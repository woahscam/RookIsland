...
//Player player, WorldObject object, int interfaceId, Item item) {
	...
	public boolean handleItemOnObject(Player player, WorldObject object, int interfaceId, Item item) {
		...
		/*
		 * case "anvil": for (int index = 0; index < Smithing.BARS[1].length; index++) {
		 * if (Smithing.BARS[1][index] == item.getId()) {
		 * Smithing.sendForgingInterface(player, index, true); break; } } return true;
		 */
		}
		return true;
	}
		...
		if (dungeon.isAtRewardsScreen()) {
			...
			if (interfaceId == 950 && componentId == 24) {
				...
				if (componentId == 2)
					...
					OldMagicSystem.processDungSpell(player, componentId, packetId);
				return false;
			}
			return true;
		}
	...
	public void addGatestone() {
		if (!OldMagicSystem.checkRunes(player, true, true, 17789, 3))
			return;
	...
	private void stoneTeleport(boolean group) {
		...
		OldMagicSystem.sendTeleportSpell(player, 13288, 13285, 2516, 2517, group ? 64 : 32, 0, tile, 2, false,
				OldMagicSystem.MAGIC_TELEPORT, group ? DungeonConstants.GROUP_GATESTONE_RUNES : new int[] {});
		player.resetReceivedDamage();
		player.unlock();
		// TODO player.setCantWalk(true);
	...
	private void portalGroupStoneTeleport() {
		OldMagicSystem.sendTeleportSpell(player, 14279, 13285, 2518, 2517, 1, 0, dungeon.getGroupGatestone(), 1, false,
				OldMagicSystem.OBJECT_TELEPORT);
	}
	...
	private boolean canCreateGatestone() {
		if (player.getInventory().containsItem(DungeonConstants.GATESTONE, 1)) {
			...
			return false;
		} else if (!OldMagicSystem.checkSpellLevel(player, 32))
			return false;
		else if (!OldMagicSystem.checkRunes(player, false, true, 17789, 3))
			return false;
		return true;
	}
