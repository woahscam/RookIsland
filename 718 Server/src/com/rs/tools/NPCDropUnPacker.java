 ...
 * 
 */
public final class NPCDropUnPacker {
		...
		for (Integer npcId : DROP_TABLES.keySet()) {
			...
			for (Drop drop : table) {
				StringBuilder sb = new StringBuilder("[").append(npcId).append("]: ").append(drop.getItemId())
						.append(", ").append(drop.getStringRate()).append(", ").append(drop.getMinAmount()).append(", ")
		...
		try {
			...
			for (int i = 0; i < dropSize; i++) {
				...
				for (int d = 0; d < drops.length; d++) {
					if (buffer.get() == 0) {
						drops[d] = new Drop(buffer.getShort() & 0xffff, buffer.toString(), buffer.getInt(),
								buffer.getInt());
					} else {
						drops[d] = new Drop(0, null, 0, 0);
					}
				}
				DROP_TABLES.put(npcId, drops);
			}
