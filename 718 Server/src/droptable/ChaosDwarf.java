...
public final class ChaosDwarf extends MobRewardNodeBuilder {
	...
	public void populate(Player player) {
		...
		case -1:
		case COMMON:
			dissectNodeBatch(1, node(STEEL_FULL_HELM, 1), 
			...
			break;
		case UNCOMMON:
			dissectNodeBatch(1, nodeB(CHAOS_RUNE, 10, 37), 
			...
			break;
		case RARE:
			dissectNodeBatch(1, node(BLACK_FULL_HELM, 1), 
					nodeB(DEATH_RUNE, 3, 6), 
					node(GRIMY_RANARR, 1));
			break;
		case VERY_RARE:
			dissectNodeBatch(1, node(DRAGON_PICKAXE, 1));
			break;
		}
		addObj(BONES, 1);
