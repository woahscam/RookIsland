...
public final class Basilisk extends MobRewardNodeBuilder {
	...
	public void populate(Player player) {
		...
		case -1:
		case COMMON:
			dissectNodeBatch(1, nodeB(WATER_RUNE, 75, 150, 175), 
					node(STEEL_BATTLEAXE, 1), 
					node(COINS, 11, 495));
			break;
		case UNCOMMON:
			dissectNodeBatch(1, nodeB(NATURE_RUNE, 15, 30, 37), 
					node(CHAOS_RUNE, 12), 
					node(LAW_RUNE, 3), 
			...
			break;
		case RARE:
			dissectNodeBatch(1, node(MYSTIC_HAT_LIGHT, 1), 
					node(GRIMY_RANARR, 1), 
					node(GRIMY_IRIT, 1), 
			...
			break;
		case VERY_RARE:
			dissectNodeBatch(1, node(MYSTIC_BOOTS_LIGHT, 1), 
