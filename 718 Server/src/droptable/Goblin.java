...
public final class Goblin extends MobRewardNodeBuilder {
	...
	public void populate(Player player) {
		...
		case -1:
		case COMMON:
			dissectNodeBatch(1, node(SLING, 1), node(IRON_DAGGER, 1), node(BRONZE_ARROW, 16), node(BRONZE_BOLTS, 8), node(BRONZE_MED_HELM, 1), node(BRONZE_SQ_SHIELD, 1),
		...
		case RARE:
		case VERY_RARE:
			dissectNodeBatch(1, node(RED_CAPE, 1), node(IRON_FULL_HELM, 1), node(LEATHER_BODY, 1), node(BRONZE_AXE, 1), node(BRONZE_JAVELIN, 5), node(BRONZE_SCIMITAR, 1),
			...
			break;
		case UNCOMMON:
			dissectNodeBatch(1, node(BRASS_NECKLACE, 1), node(IRON_ARROW, 3, 6), node(BRONZE_CHAINBODY, 1), node(BRONZE_FULL_HELM, 1), node(BRONZE_KITESHIELD, 1),
