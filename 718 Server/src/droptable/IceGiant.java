...
public final class IceGiant extends MobRewardNodeBuilder {
	...
	public void populate(Player player) {
		...
		case -1:
		case COMMON:
			dissectNodeBatch(1, node(BLACK_KITESHIELD, 1), 
			...
			break;
		case UNCOMMON:
			dissectNodeBatch(1, node(ADAMANT_ARROW, 5), 
					node(LAW_RUNE, 3), 
					node(NATURE_RUNE, 6), 
			...
			break;
		case RARE:
			dissectNodeBatch(1, node(MITHRIL_SQ_SHIELD, 1), 
					node(MITHRIL_MACE, 1), 
					node(BLOOD_RUNE, 2, 4), 
					node(DEATH_RUNE, 2, 4), 
					node(AVANTOE_SEED, 1), 
			...
			break;
		case VERY_RARE:
			dissectNodeBatch(1, node(KWUARM_SEED, 1), 
