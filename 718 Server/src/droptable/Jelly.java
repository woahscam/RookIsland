...
public final class Jelly extends MobRewardNodeBuilder {
	...
	public void populate(Player player) {
		...
		case -1:
		case COMMON:
			dissectNodeBatch(1, node(STEEL_BATTLEAXE, 1), 
					node(STEEL_2H_SWORD, 1), 
					node(STEEL_AXE, 1), 
					node(COINS, 11, 497));
			break;
		case UNCOMMON:
			dissectNodeBatch(1, node(CHAOS_RUNE, 15), 
					node(DEATH_RUNE, 5), 
			...
			break;
		case RARE:
			dissectNodeBatch(1, node(WATER_RUNE, 5, 21), 
					node(BLOOD_RUNE, 5), 
			...
			break;
		case VERY_RARE:
			dissectNodeBatch(1, node(RUNE_BATTLEAXE, 1));
			break;
		}
