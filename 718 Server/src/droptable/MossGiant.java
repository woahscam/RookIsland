...
public final class MossGiant extends MobRewardNodeBuilder {
	...
	public void populate(Player player) {
		...
		case COMMON:
			dissectNodeBatch(1, node(IRON_ARROW, 15), 
					node(STEEL_ARROW, 30), 
					node(MAGIC_STAFF, 1), 
			...
			break;
		case UNCOMMON:
			dissectNodeBatch(1, node(STEEL_KITESHIELD, 1), 
					node(BLOOD_RUNE, 1), 
					node(CHAOS_RUNE, 7), 
					node(LAW_RUNE, 3, 6), 
					node(COAL, 1), 
			...
			break;
		case RARE:
			dissectNodeBatch(1, node(ADAMANT_SWORD, 1), 
			...
			break;
		case VERY_RARE:
			dissectNodeBatch(1, node(SOUL_RUNE, 20), 
